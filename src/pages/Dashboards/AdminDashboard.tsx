...
const recentSignals = [
  {
    ...
    detail: "32% higher than avg.",
  },
  {
    type: "neutral",
    title: "COD conversion improved.",
    detail: "Drop in cart abandonment.",
  },
  {
    type: "star",
    title: "High repeat buyers in Kerala.",
    detail: "Region driving loyalty.",
  },
];
      ...
      <div className="max-w-[1600px] mx-auto space-y-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {stats.map((item) => (
            <div
              key={item.label}
              className="group relative overflow-hidden rounded-xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm hover:border-zinc-300 dark:hover:border-zinc-700 transition-all"
            >
              <div className="flex justify-between items-start">
                ...
                <div className="flex flex-col items-end gap-3">
                  <div
                    className={`p-2 rounded-lg ${item.trend === "up" ? "bg-zinc-100 dark:bg-zinc-900 text-black dark:text-white" : "bg-zinc-100 dark:bg-zinc-900 text-zinc-500"}`}
                  >
                    <item.icon className="h-5 w-5" />
                  </div>
                  <span
                  ...
                  >
                    ...
                    <Bar dataKey="value" radius={[4, 4, 0, 0]} barSize={40}>
                      {chartData.map((_entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          className="fill-zinc-900 dark:fill-zinc-100 hover:opacity-80 transition-opacity"
                        />
                      ))}
                    </Bar>
                  </BarChart>
            ...
            <div className="rounded-xl bg-white dark:bg-zinc-950 p-6 shadow-sm border border-zinc-200 dark:border-zinc-800">
              ...
              <div className="space-y-4">
                {quickInsights.map((item) => (
                  <div
                    key={item.label}
                    className="flex justify-between items-center p-3 rounded-lg bg-gray-50 dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800"
                  >
                    <div>
                ...
                {recentSignals.map((signal, i) => (
                  <div key={i} className="flex gap-4 group">
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-2 h-2 rounded-full ${signal.type === "up" ? "bg-emerald-500" : signal.type === "star" ? "bg-amber-500" : "bg-blue-500"} ring-4 ring-white dark:ring-black`}
                      />
                      {i !== recentSignals.length - 1 && (
                        <div className="w-px h-full bg-zinc-100 dark:bg-zinc-800 my-1" />
                      )}
                    </div>
                    <div className="pb-2">
                      <p className="text-xs font-bold text-zinc-900 dark:text-white leading-tight group-hover:text-indigo-600 transition-colors">
                        {signal.title}
                      </p>
