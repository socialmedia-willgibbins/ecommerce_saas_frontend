...
export const ListAllOrdersTable = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // - Admin users: only see orders for their products
      ...
      // Filter out "pending" status orders (payment not completed)
                                ...
                                {order.order_details.map((detail) => (
                                  ...
                                  >
                                    <td className="px-4 py-3">
                                      <div className="flex items-center gap-3">
                                        <div className="h-8 w-8 rounded border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 flex items-center justify-center overflow-hidden">
                                          {detail.product_details.images[0]
                                            ?.url ? (
                                            <img
                                              src={
                                                detail.product_details.images[0]
                                    ...
                                    <td className="px-4 py-3 font-mono text-zinc-500">
                                      {detail.product_details.product_code}
                                    </td>
                                    <td className="px-4 py-3 font-bold text-zinc-900 dark:text-white">
                                      {detail.quantity}
                                    </td>
                                    <td className="px-4 py-3 text-zinc-600 dark:text-zinc-300">
                                      {formatPrice(detail.price_at_purchase)}
                                    </td>
                                    <td className="px-4 py-3 font-bold text-zinc-900 dark:text-white">
                                      {formatPrice(
                                        (
                                          parseFloat(detail.price_at_purchase) *
                                          detail.quantity
                                        ).toString(),
                                      )}
                                    </td>
